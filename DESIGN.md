...
| Two accounts choose the same order ID | **Known limit, and larger than it looks.** Order IDs are venue-global, so an ID one account has resting is refused to another as a duplicate: a client can deny another an ID and probe which are in use. The obvious fix -- key the reservation table by `(account, order_id)` -- does not stand alone. The matching engine reports a fill as `maker_order_id` and nothing else, so the pipeline could not build the key for the maker side of a trade without the engine also reporting the owner, which widens `Fill` on the one struct produced once per execution: a 2,000-level sweep makes 2,000 of them. The alternative, venue-assigned internal IDs behind a `(account, client_order_id)` map, adds a lookup to every command. Neither is a small change and neither has been measured, so the limit stands rather than being closed by guesswork. What it is not: a path to anyone else's balance. The account binding is checked before sequencing, so a colliding ID is refused, never applied to the wrong account |
